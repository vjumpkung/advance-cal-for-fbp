...
from workflow_execution.graph import (
    get_input_info,
    ExecutionList,
    DynamicWorkflow,
    ExecutionBlocker,
)
from workflow_execution.graph_utils import is_link, GraphBuilder
from workflow_execution.caching import (
    HierarchicalCache,
    LRUCache,
...
class IsChangedCache:
    def __init__(self, dynworkflow, outputs_cache):
        self.dynworkflow = dynworkflow
...
def get_input_data(
    inputs, class_def, unique_id, outputs=None, dynworkflow=None, extra_data={}
):
    ...
    if "hidden" in valid_inputs:
        ...
        for x in h:
            if h[x] == "PROMPT":
                input_data_all[x] = [
                    (
                        dynworkflow.get_original_workflow()
                        if dynworkflow is not None
                        else {}
                    )
                ]
            if h[x] == "DYNPROMPT":
                input_data_all[x] = [dynworkflow]
...
def execute(
    ...
    real_node_id = dynworkflow.get_real_node_id(unique_id)
    display_node_id = dynworkflow.get_display_node_id(unique_id)
    ...
    try:
        if unique_id in pending_subgraph_results:
            ...
            input_data_all, missing_keys = get_input_data(
                inputs, class_def, unique_id, caches.outputs, dynworkflow, extra_data
            )
            if server.client_id is not None:
            ...
            for i in range(len(output_data)):
                ...
                if new_graph is None:
                    ...
                    for node_id in new_graph.keys():
                        if dynworkflow.has_node(node_id):
                    ...
                    for node_id, node_info in new_graph.items():
                        ...
                        dynworkflow.add_ephemeral_node(
...
class WorkflowExecutor:
    ...
    def handle_execution_error(
        self, workflow_id, workflow, current_outputs, executed, error, ex
    ):
    ...
    def execute(self, workflow, workflow_id, extra_data={}, execute_outputs=[]):
        ...
        self.status_messages = []
        self.add_message(
            "execution_start", {"workflow_id": workflow_id}, broadcast=False
        )

        dynamic_workflow = DynamicWorkflow(workflow)
        is_changed_cache = IsChangedCache(dynamic_workflow, self.caches.outputs)
        for cache in self.caches.all:
            cache.set_workflow(dynamic_workflow, workflow.keys(), is_changed_cache)
            cache.clean_unused()
        ...
        self.add_message(
            "execution_cached",
            {"nodes": cached_nodes, "workflow_id": workflow_id},
            broadcast=False,
        )
        pending_subgraph_results = {}
        executed = set()
        execution_list = ExecutionList(dynamic_workflow, self.caches.outputs)
        ...
        while not execution_list.is_empty():
            ...
            if error is not None:
                self.handle_execution_error(
                    workflow_id,
                    dynamic_workflow.original_workflow,
                    current_outputs,
                    executed,
                    error,
                    ex,
            ...
            self.add_message(
                "execution_success", {"workflow_id": workflow_id}, broadcast=False
            )
    ...
    for x in valid_inputs:
        ...
        if isinstance(val, list):
            ...
            if (
                ...
                continue
            try:
                r = validate_inputs(workflow, o_id, validated)
                if r[0] is False:
...
def validate_workflow(workflow):
    outputs = set()
    for x in workflow:
        if "class_type" not in workflow[x]:
            error = {
    ...
    if len(outputs) == 0:
        error = {
            "type": "workflow_no_outputs",
            "message": "Workflow has no outputs",
            "details": "",
            "extra_info": {},
        }
        return (False, error, [], [])
    ...
    for o in outputs:
        ...
        reasons = []
        try:
            m = validate_inputs(workflow, o, validated)
            valid = m[0]
            reasons = m[1]
        except Exception as ex:
            typ, _, tb = sys.exc_info()
        ...
        if valid is True:
            ...
            logging.error(f"Failed to validate workflow for output {o}:")
            if len(reasons) > 0:
    ...
    if len(good_outputs) == 0:
        ...
        error = {
            "type": "workflow_outputs_failed_validation",
            "message": "Workflow outputs failed validation",
            "details": errors_list,
            "extra_info": {},
        }
...
class WorkflowQueue:
    ...
    def task_done(
        ...
        with self.mutex:
            ...
            self.history[workflow[1]] = {
                "workflow": workflow,
                "outputs": {},
                "status": status_dict,
            }
            self.history[workflow[1]].update(history_result)
            self.server.queue_updated()
    ...
    def get_history(self, workflow_id=None, max_items=None, offset=-1):
        with self.mutex:
            if workflow_id is None:
                ...
                for k in self.history:
                    ...
                    i += 1
                return out
            elif workflow_id in self.history:
                return {workflow_id: copy.deepcopy(self.history[workflow_id])}
            else:
                return {}
