...
class DynamicWorkflow:
    def __init__(self, original_workflow):
        # The original workflow provided by the user
        self.original_workflow = original_workflow
        # Any extra pieces of the graph created during execution
        self.ephemeral_workflow = {}
        self.ephemeral_parents = {}
        self.ephemeral_display = {}

    def get_node(self, node_id):
        if node_id in self.ephemeral_workflow:
            return self.ephemeral_workflow[node_id]
        if node_id in self.original_workflow:
            return self.original_workflow[node_id]
        raise NodeNotFoundError(f"Node {node_id} not found")

    def has_node(self, node_id):
        return node_id in self.original_workflow or node_id in self.ephemeral_workflow

    def add_ephemeral_node(self, node_id, node_info, parent_id, display_id):
        self.ephemeral_workflow[node_id] = node_info
    ...
    def all_node_ids(self):
        return set(self.original_workflow.keys()).union(
            set(self.ephemeral_workflow.keys())
        )
...
class TopologicalSort:
    ...
    def make_input_strong_link(self, to_node_id, to_input):
        inputs = self.dynworkflow.get_node(to_node_id)["inputs"]
    ...
    def add_node(self, unique_id, include_lazy=False, subgraph_nodes=None):
        ...
        inputs = self.dynworkflow.get_node(unique_id)["inputs"]
        for input_name in inputs:
            ...
            if is_link(value):
                ...
                if subgraph_nodes is not None and from_node_id not in subgraph_nodes:
                    continue
                input_type, input_category, input_info = self.get_input_info(
                    unique_id, input_name
                )
                is_lazy = (
...
class ExecutionList(TopologicalSort):
    ...
    def __init__(self, dynworkflow, output_cache):
        super().__init__(dynworkflow)
    ...
    def stage_node_execution(self):
        ...
        if len(available) == 0:
            ...
            for node_id in cycled_nodes:
                display_node_id = self.dynworkflow.get_display_node_id(node_id)
